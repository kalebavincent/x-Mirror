...
class TaskConfig:
    ...
    async def is_token_exists(self, path, status):
        if is_rclone_path(path):
            ...
            if not await aiopath.exists(config_path):
                raise ValueError(f"Config Rclone : {config_path} n'existe pas !")
        elif (
            status == "dl"
            ...
            if not await aiopath.exists(token_path):
                raise ValueError(f"AUCUN TOKEN ! {token_path} n'existe pas !")
    ...
    async def before_start(self):
        ...
        if not self.is_leech:
            ...
            if not self.up_dest:
                raise ValueError("Aucune destination de téléchargement !")
            if is_gdrive_id(self.up_dest):
                ...
                raise ValueError("Mauvaise destination de téléchargement !")
            ...
            if self.up_dest == "rcl":
                if self.is_clone:
                    if not is_rclone_path(self.link):
                        raise ValueError(
                            "Vous ne pouvez pas cloner depuis différents types d'outils"
                        )
                ...
                if is_gdrive_link(self.link) and self.get_token_path(
                    ...
                    raise ValueError("Vous devez utiliser le même token pour cloner !")
                elif is_rclone_path(self.link) and self.get_config_path(
                    self.link
                ) != self.get_config_path(self.up_dest):
                    raise ValueError("Vous devez utiliser la même config pour cloner !")
        else:
            self.up_dest = (
                self.up_dest
                ...
                if not self.user_transmission or self.hybrid_leech:
                    ...
                    if chat is None:
                        if self.user_transmission:
                            ...
                            raise ValueError("Chat introuvable !")
                    else:
                        ...
                        if chat.type.name in ["SUPERGROUP", "CHANNEL", "GROUP"]:
                            ...
                            if (
                                ...
                                if not self.user_transmission:
                                    raise ValueError(
                                        "Vous n'avez pas suffisamment de privilèges dans ce chat !"
                                    )
                                else:
                                    self.hybrid_leech = False
                        else:
                            ...
                            if res["@type"] != "ok":
                                raise ValueError(
                                    f"Démarrez le bot et réessayez ! Erreur : {res['message']}"
                                )
            elif (
    ...
    async def run_multi(self, input_list, obj):
        ...
        if self.multi_tag and self.multi_tag not in multi_tags:
            await send_message(
                self.message, f"{self.tag} La tâche multiple a été annulée !"
            )
        ...
        if len(self.bulk) != 0:
            ...
            msgts = " ".join(msg)
            if self.multi > 2:
                msgts += f"\nAnnuler Multi : <code>/{BotCommands.CancelTaskCommand[1]} {self.multi_tag}</code>"
    ...
    async def init_bulk(self, input_list, bulk_start, bulk_end, obj):
        try:
            ...
            await send_message(
                self.message,
                f"Répondez à un fichier texte ou à un message Telegram contenant des liens séparés par des nouvelles lignes ! {e}",
            )
    ...
    async def proceed_extract(self, dl_path, gid):
        ...
        t_path = dl_path
        sevenz = SevenZ(self)
        LOGGER.info(f"Extraction : {self.name}")
        async with task_dict_lock:
        ...
        if self.proceed_count == 0:
            LOGGER.info("Aucun fichier à extraire !")
            ...
            for ffmpeg_cmd in cmds:
                ...
                if await aiopath.isfile(dl_path):
                    ...
                    if not checked:
                        ...
                        self.progress = True
                    LOGGER.info(f"Exécution de la commande ffmpeg pour : {file_path}")
                    for index in input_indexes:
                        if cmd[index + 1].startswith("mltb"):
                    ...
                    for dirpath, _, files in await sync_to_async(
                        ...
                        for file_ in files:
                            ...
                            if not checked:
                                ...
                                await cpu_eater_lock.acquire()
                                self.progress = True
                            LOGGER.info(f"Exécution de la commande ffmpeg pour : {f_path}")
        ...
        def perform_substitution(name, substitutions):
            for substitution in substitutions:
                ...
                try:
                    ...
                    LOGGER.error(
                        f"Erreur de substitution : motif : {pattern} res : {res}. Erreur : {e}"
                    )
                    return False
                if len(name.encode()) > 255:
                    LOGGER.error(f"Substitution : {name} est trop long")
                    return False
            return name
    ...
    async def generate_screenshots(self, dl_path):
        ...
        if self.is_file:
            if (await get_document_type(dl_path))[0]:
                LOGGER.info(f"Création de captures d'écran pour : {dl_path}")
        ...
        if self.files_to_proceed:
            ...
            async with cpu_eater_lock:
                ...
                for f_path, f_type in self.files_to_proceed.items():
                    self.proceed_count += 1
                    LOGGER.info(f"Conversion : {f_path}")
                    if self.is_file:
                        self.subsize = self.size
                    else:
                        self.subsize = await get_path_size(f_path)
    ...
    async def generate_sample_video(self, dl_path, gid):
        ...
        if self.files_to_proceed:
            ffmpeg = FFMpeg(self)
            async with task_dict_lock:
                task_dict[self.mid] = FFmpegStatus(self, ffmpeg, gid, "Échantillon Vidéo")
            self.progress = False
            async with cpu_eater_lock:
                self.progress = True
                LOGGER.info(f"Création d'un échantillon vidéo : {self.name}")
                for f_path, file_ in self.files_to_proceed.items():
                    ...
                    if res and self.is_file:
                        ...
                        await gather(
                            ...
                            move(res, f"{new_folder}/ÉCHANTILLON.{file_}"),
                        )
                        return new_folder
        return dl_path
    ...
    async def proceed_split(self, dl_path, gid):
        ...
        if self.files_to_proceed:
            ...
            LOGGER.info(f"Division : {self.name}")
