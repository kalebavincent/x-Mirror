...
def get_readable_time(seconds: int):
    periods = [("j", 86400), ("h", 3600), ("m", 60), ("s", 1)]
    result = ""
...
async def get_readable_message(sid, is_user, page_no=1, status="Toutes", page_step=1):
    msg = ""
    button = None
    ...
    for index, task in enumerate(
        ...
        display_status = STATUS_TRANSLATION.get(tstatus, tstatus)

        if task.listener.is_super_chat:
            msg += f"<b>{index + start_position}.<a href='{task.listener.message.link}'>{display_status}</a>: </b>"
        else:
            msg += f"<b>{index + start_position}.{display_status}: </b>"
        ...
        if (
            ...
            if task.listener.subname:
                ...
                subsize = ""
                count = ""
            msg += f"\n<b>Traîté:</b> {task.processed_bytes()}{subsize}"
            if count:
                msg += f"\n<b>Compte:</b> {count}"
            msg += f"\n<b>Taille:</b> {task.size()}"
            msg += f"\n<b>Vitesse:</b> {task.speed()}"
            msg += f"\n<b>ETA:</b> {task.eta()}"
            if (
            ...
            msg += f"\n<b>Taille: </b>{task.size()}"
            msg += f"\n<b>Vitesse: </b>{task.seed_speed()}"
            msg += f"\n<b>Envoyé: </b>{task.uploaded_bytes()}"
            msg += f"\n<b>Ratio: </b>{task.ratio()}"
            msg += f" | <b>Temps: </b>{task.seeding_time()}"
        else:
            msg += f"\n<b>Taille: </b>{task.size()}"
    ...
    if len(msg) == 0:
        if status == "Toutes":
            ...
            display_status = STATUS_TRANSLATION.get(status, status)
            msg = f"Aucune tâche {display_status} active !\n\n"
    buttons = ButtonMaker()
    if not is_user:
        buttons.data_button("📜", f"status {sid} ov", position="header")
    if len(tasks) > STATUS_LIMIT:
        msg += f"<b>Page:</b> {page_no}/{pages} | <b>Tâches:</b> {tasks_no} | <b>Pas:</b> {page_step}\n"
        buttons.data_button("<<", f"status {sid} pre", position="header")
        buttons.data_button(">>", f"status {sid} nex", position="header")
        if tasks_no > 30:
            for i in [1, 2, 4, 6, 8, 10, 15]:
                buttons.data_button(i, f"status {sid} ps {i}", position="footer")
    if status != "Toutes" or tasks_no > 20:
        for label, status_value in list(STATUSES.items()):
            if status_value != status:
                buttons.data_button(label, f"status {sid} st {status_value}")
    buttons.data_button("♻️", f"status {sid} ref", position="header")
    button = buttons.build_menu(8)
    msg += f"<b>CPU:</b> {cpu_percent()}% | <b>LIBRE:</b> {get_readable_file_size(disk_usage(DOWNLOAD_DIR).free)}"
