...
class TaskListener(TaskConfig):
    ...
    async def on_download_complete(self):
                    ...
                    async with task_dict_lock:
                        ...
                        if (
                            ...
                            if self.same_dir[self.folder_name]["total"] > 1:
                                ...
                                LOGGER.info(f"Déplacement des fichiers de {self.mid} vers {des_id}")
        ...
        async with task_dict_lock:
            ...
            gid = download.gid()
        LOGGER.info(f"Téléchargement terminé : {self.name}")
        ...
        if multi_links:
            ...
            await self.on_upload_error(
                f"{self.name} Téléchargé !\n\nEn attente de la fin des autres tâches..."
            )
            return
        elif self.same_dir:
            self.seed = False
        ...
        if self.seed:
            ...
            LOGGER.info(f"Raccourci créé : {dl_path} -> {up_path}")
        else:
            up_dir = self.dir
            up_path = dl_path
        ...
        await start_from_queued()
        if add_to_queue:
            LOGGER.info(f"Ajouté à la file d'attente/upload : {self.name}")
            async with task_dict_lock:
                task_dict[self.mid] = QueueStatus(self, gid, "Up")
            await event.wait()
            if self.is_cancelled:
                return
            LOGGER.info(f"Démarrage depuis la file d'attente/upload : {self.name}")

        self.size = await get_path_size(up_dir)

        if self.is_leech:
            LOGGER.info(f"Nom du Leech : {self.name}")
            ...
            LOGGER.info(f"Nom de l'upload Gdrive : {self.name}")
            ...
            LOGGER.info(f"Nom de l'upload Rclone : {self.name}")
    ...
    async def on_upload_complete(
        ...
        msg = f"<b>Nom : </b><code>{escape(self.name)}</code>\n\n<b>Taille : </b>{get_readable_file_size(self.size)}"
        LOGGER.info(f"Tâche terminée : {self.name}")
        if self.is_leech:
            msg += f"\n<b>Total des fichiers : </b>{folders}"
            if mime_type != 0:
                msg += f"\n<b>Fichiers corrompus : </b>{mime_type}"
            msg += f"\n<b>cc : </b>{self.tag}\n\n"
            if not files:
            ...
            if (
                ...
                buttons = ButtonMaker()
                if link:
                    buttons.url_button("☁️ Lien Cloud", link)
                else:
                    msg += f"\n\nChemin : <code>{rclone_path}</code>"
                if rclone_path and Config.RCLONE_SERVE_URL and not self.private_link:
                    ...
                    if mime_type == "Dossier":
                        share_url += "/"
                    buttons.url_button("🔗 Lien Rclone", share_url)
                if not rclone_path and dir_id:
                    ...
                    if INDEX_URL:
                        ...
                        buttons.url_button("⚡ Lien Index", share_url)
                        if mime_type.startswith(("image", "video", "audio")):
                            share_urls = f"{INDEX_URL}findpath?id={dir_id}&view=true"
                            buttons.url_button("🌐 Lien de visualisation", share_urls)
    ...
    async def on_download_error(self, error, button=None):
        ...
        msg = f"{self.tag} Téléchargement : {escape(str(error))}"
        await send_message(self.message, msg, button)
        if count == 0:
            await self.clean()
        else:
