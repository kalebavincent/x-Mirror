...
class DirectListener:
    ...
    async def download(self, contents):
        ...
        for content in contents:
            ...
            try:
                ...
                self._failed += 1
                LOGGER.error(f"Impossible de télécharger {filename} : {e}")
                continue
            ...
            while True:
                ...
                if error_message := self.download_task.get("errorMessage"):
                    self._failed += 1
                    LOGGER.error(
                        f"Impossible de télécharger {aria2_name(self.download_task)} : {error_message}"
                    )
                    await TorrentManager.aria2_remove(self.download_task)
                    break
        ...
        if self._failed == len(contents):
            await self.listener.on_download_error("Échec du téléchargement de tous les fichiers !")
            return
    ...
    async def cancel_task(self):
        ...
        LOGGER.info(f"Annulation du téléchargement : {self.listener.name}")
        await self.listener.on_download_error("Téléchargement annulé par l'utilisateur !")
        if self.download_task:
            await TorrentManager.aria2_remove(self.download_task)