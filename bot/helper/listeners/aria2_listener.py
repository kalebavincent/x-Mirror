...
async def _on_download_started(api, data):
    ...
    if is_metadata(download):
        LOGGER.info(f"onDownloadStarted: {gid} MÉTADONNÉES")
        await sleep(1)
        if task := await get_task_by_gid(gid):
            task.listener.is_torrent = True
            if task.listener.select:
                metamsg = "Téléchargement des métadonnées, attendez puis vous pourrez sélectionner les fichiers. Utilisez un fichier torrent pour éviter cette attente."
...
async def _on_download_complete(api, data):
    ...
    if download.get("followedBy", []):
        ...
        LOGGER.info(f"Gid changé de {gid} à {new_gid}")
        if task := await get_task_by_gid(new_gid):
            ...
            if Config.BASE_URL and task.listener.select:
                ...
                msg = "Téléchargement en pause. Choisissez les fichiers puis appuyez sur Terminer la sélection."
        ...
        if task := await get_task_by_gid(gid):
            ...
            if hasattr(task, "seeding") and task.seeding:
                LOGGER.info(
                    f"Annulation du Seed: {aria2_name(download)} onDownloadComplete"
                )
                await TorrentManager.aria2_remove(download)
                await task.listener.on_upload_error(
                    f"Seed stoppé avec Ratio: {task.ratio()} et Temps: {task.seeding_time()}"
                )
    else:
    ...
    if task := await get_task_by_gid(gid):
        ...
        if task.listener.seed:
            try:
                ...
                LOGGER.error(
                    f"{e} Impossible de seeder car l'option globale seed-time=0 est définie sans temps de seed spécifique pour ce torrent GID: {gid}"
                )
        else:
            try:
        ...
        if (
            ...
            LOGGER.info(f"Annulation du Seed: {aria2_name(download)}")
            await TorrentManager.aria2_remove(download)
            await task.listener.on_upload_error(
                f"Seed stoppé avec Ratio: {task.ratio()} et Temps: {task.seeding_time()}"
            )
            ...
            LOGGER.info(f"Seed démarré: {aria2_name(download)} - Gid: {gid}")
...
async def _on_download_stopped(_, data):
    ...
    await sleep(4)
    if task := await get_task_by_gid(gid):
        await task.listener.on_download_error("Torrent mort !")
...
async def _on_download_error(api, data):
    ...
    with suppress(TimeoutError, ClientError, Exception):
        ...
        LOGGER.info(f"Erreur de téléchargement: {error}")
    if options.get("follow-torrent", "") == "false":
        return
    if task := await get_task_by_gid(gid):
