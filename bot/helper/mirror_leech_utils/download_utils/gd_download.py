...
async def add_gd_download(listener, path):
    ...
    if add_to_queue:
        LOGGER.info(f"Ajouté à la file/Download: {listener.name}")
    ...
    if add_to_queue:
        LOGGER.info(f"Démarrage du téléchargement depuis la file: {listener.name}")
    else:
        LOGGER.info(f"Téléchargement depuis Google Drive: {listener.name}")
        await listener.on_download_start()
        if listener.multi <= 1:
            await send_status_message(listener.message)
