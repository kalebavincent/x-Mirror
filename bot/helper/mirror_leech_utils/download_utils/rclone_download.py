...
async def add_rclone_download(listener, path):
    ...
    if rclone_select:
        ...
        if res[2] != 0:
            if res[2] != -9:
                msg = f"Erreur: Problème lors de la récupération des stats rclone. Chemin: {remote}:{listener.link}. Erreur: {res[1][:4000]}"
                await listener.on_download_error(msg)
            return
        try:
            rsize = loads(res[0])
        except Exception as err:
            await listener.on_download_error(f"Erreur de chargement JSON RcloneDownload: {err}")
            return
        ...
        if res1[2] != 0 or res2[2] != 0:
            if res1[2] != -9:
                ...
                msg = f"Erreur: Problème lors de la récupération des stats rclone. Chemin: {remote}:{listener.link}. Erreur: {err[:4000]}"
                await listener.on_download_error(msg)
            return
        try:
            rstat = loads(res1[0])
    ...
    if add_to_queue:
        LOGGER.info(f"Ajouté à la file/téléchargement: {listener.name}")
    ...
    if add_to_queue:
        LOGGER.info(f"Démarrage du téléchargement en file avec rclone: {listener.link}")
    else:
        await listener.on_download_start()
        if listener.multi <= 1:
            await send_status_message(listener.message)
        LOGGER.info(f"Téléchargement avec rclone: {listener.link}")
