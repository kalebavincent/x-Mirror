...
async def configureDownload(_, query, obj):
    ...
    if data[1] == "sdone":
        ...
        await edit_message(message, "Tâche annulée.")
...
class JDownloaderHelper:
    ...
    async def _event_handler(self):
        ...
        try:
            ...
            await edit_message(self._reply_to, "Délai dépassé. Tâche annulée !")
    ...
    async def wait_for_configurations(self):
        buttons = ButtonMaker()
        buttons.url_button("Sélectionner", "https://my.jdownloader.org")
        buttons.data_button("Terminer la sélection", "jdq sdone")
        buttons.data_button("Annuler", "jdq cancel")
        button = buttons.build_menu(2)
        msg = f"Retirez les fichiers indésirables ou modifiez les variantes depuis le site myJdownloader pour <b>{self.listener.name}</b>.\nNe le démarrez pas manuellement !\n\nAprès avoir terminé, appuyez sur Terminer la sélection !\nDélai : 10 min"
    ...
    try:
        async with jd_listener_lock:
            ...
            while (time() - start_time) < 90:
                ...
                error = (
                    name
                    or "Téléchargement non ajouté ! Problème avec JDownloader ou le site !"
                )
        ...
        if listener.select:
            if not await JDownloaderHelper(listener).wait_for_configurations():
                ...
                if not online_packages:
                    raise MYJDException(
                        "Sélection : Ce téléchargement a été supprimé manuellement !"
                    )
                async with jd_listener_lock:
                    jd_downloads[gid]["ids"] = online_packages
        ...
        if add_to_queue:
            LOGGER.info(f"Ajouté à la file d'attente/téléchargement : {listener.name}")
            ...
            online_packages = await get_online_packages(path)
            if not online_packages:
                raise MYJDException("File d'attente : Ce téléchargement a été supprimé manuellement !")
            async with jd_listener_lock:
                jd_downloads[gid]["ids"] = online_packages
        ...
        if not online_packages:
            ...
            if not online_packages:
                raise MYJDException(
                    "Linkgrabber : Ce téléchargement a été supprimé manuellement !"
                )
            await jdownloader.device.linkgrabber.move_to_downloadlist(
                package_ids=online_packages
            )
        ...
        if not online_packages:
            raise MYJDException(
                "Liste de téléchargement : Ce téléchargement a été supprimé manuellement !"
            )
        ...
        if add_to_queue:
            LOGGER.info(f"Démarrage depuis la file d'attente : {listener.name}")
        else:
            LOGGER.info(f"Téléchargement avec JDownloader : {listener.name}")
            await listener.on_download_start()
            if listener.multi <= 1:
                await send_status_message(listener.message)
