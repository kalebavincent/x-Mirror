...
async def add_qb_torrent(listener, path, ratio, seed_time):
    try:
        ...
        try:
            ...
            LOGGER.error(
                f"{e}. {listener.mid}. Torrent déjà ajouté ou type de lien/fichier non supporté !"
            )
        ...
        if add_to_queue:
            LOGGER.info(f"Ajouté à la file d'attente : {tor_info.name} - Hash : {ext_hash}")
        else:
            LOGGER.info(f"Téléchargement Qbit démarré : {tor_info.name} - Hash : {ext_hash}")

        await listener.on_download_start()

        if Config.BASE_URL and listener.select:
            if listener.link.startswith("magnet:"):
                metamsg = "Téléchargement des métadonnées, attendez avant de sélectionner les fichiers. Utilisez un fichier torrent pour éviter cette attente."
        ...
        if event is not None:
            if not event.is_set():
                ...
                LOGGER.info(
                    f"Démarrage depuis la file d'attente : {tor_info.name} - Hash : {ext_hash}"
                )
            await on_download_start(f"{listener.mid}")
