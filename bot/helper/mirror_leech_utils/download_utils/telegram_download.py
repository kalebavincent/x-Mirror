...
class TelegramDownloadHelper:
    ...
    async def _on_download_start(self, file_id, from_queue):
        ...
        if not from_queue:
            ...
            LOGGER.info(f"Téléchargement depuis Telegram: {self._listener.name}")
        else:
            LOGGER.info(f"Démarrage du téléchargement en file depuis Telegram: {self._listener.name}")
    ...
    async def _download(self, message, path):
        ...
        if download is not None:
            ...
            await self._on_download_error("Erreur interne")
    ...
    async def add_download(self, message, path, session):
        ...
        if media is not None:
            ...
            if download:
                ...
                if add_to_queue:
                    LOGGER.info(f"Ajouté à la file/téléchargement: {self._listener.name}")
                    async with task_dict_lock:
                ...
                await self._on_download_error("Fichier déjà en cours de téléchargement !")
        else:
            await self._on_download_error(
                "Aucun document dans le message ! Utilisez un SuperGroup si vous essayez de télécharger avec une session utilisateur !"
            )

    async def cancel_task(self):
        self._listener.is_cancelled = True
        LOGGER.info(
            f"Annulation du téléchargement sur demande: nom: {self._listener.name} id: {self._id}"
        )
        await self._on_download_error("Arrêté par l'utilisateur !")