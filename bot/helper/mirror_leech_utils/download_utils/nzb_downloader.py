...
async def add_servers():
    ...
    if res and (servers := res["servers"]):
        ...
        raise NotLoggedIn("Configurez USENET_SERVERS dans bsetting ou config !")
    else:
        if tasks := [
    ...
    try:
        ...
        if not res["status"]:
            await listener.on_download_error(
                "Non ajouté ! Problème probable avec le lien",
            )
            return
        ...
        if add_to_queue:
            LOGGER.info(f"Ajouté à la file/téléchargement: {name} - Job_id: {job_id}")
        else:
            LOGGER.info(f"Téléchargement NZB démarré: {name} - Job_id: {job_id}")

        await listener.on_download_start()

        if Config.BASE_URL and listener.select:
            if url and name.startswith("Trying"):
                metamsg = "Récupération de l'URL, veuillez patienter avant de sélectionner les fichiers. Utilisez un fichier NZB pour éviter cette attente."
            ...
            msg = "Téléchargement en pause. Sélectionnez les fichiers puis appuyez sur 'Terminé'."
        ...
        if add_to_queue:
            ...
            LOGGER.info(
                f"Démarrage du téléchargement depuis la file: {name} - Job_id: {job_id}"
            )
    except Exception as e:
