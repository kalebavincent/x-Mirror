...
class TelegramUploader:
        ...
        if self._listener.up_dest:
            ...
            if self._sent_msg is None:
                self._sent_msg = await TgClient.user.send_message(
                    ...
                    text="Message de commande supprimé ! Ne supprimez plus le message de commande !",
                    disable_web_page_preview=True,
                    disable_notification=True,
                )
        else:
        ...
        for dirpath, _, files in natsorted(await sync_to_async(walk, self._path)):
            ...
            for file_ in natsorted(files):
                ...
                if not await aiopath.exists(self._up_path):
                    LOGGER.error(f"{self._up_path} n'existe pas ! Poursuite de l'envoi.")
                    continue
                try:
                    ...
                    if f_size == 0:
                        LOGGER.error(
                            f"{self._up_path} taille zéro, Telegram n'accepte pas les fichiers vides"
                        )
                        self._corrupted += 1
                        continue
                    if self._listener.is_cancelled:
                    ...
                    if isinstance(err, RetryError):
                        LOGGER.info(
                            f"Tentatives totales: {err.last_attempt.attempt_number}"
                        )
                        err = err.last_attempt.exception()
                    LOGGER.error(f"{err}. Chemin: {self._up_path}")
                    self._error = str(err)
                    self._corrupted += 1
                    if self._listener.is_cancelled:
                        return
            ...
            for subkey, msgs in list(value.items()):
                if len(msgs) > 1:
                    try:
                        ...
                        LOGGER.info(
                            f"Erreur lors de l'envoi du groupe média en fin de tâche : {e}"
                        )
        if self._listener.is_cancelled:
            return
        if self._total_files == 0:
            await self._listener.on_upload_error(
                "Aucun fichier à envoyer. Si vous avez défini EXCLUDED_EXTENSIONS, vérifiez que tous les fichiers ont ces extensions."
            )
            return
        if self._total_files <= self._corrupted:
            await self._listener.on_upload_error(
                f"Fichiers corrompus ou impossibles à envoyer. {self._error or 'Consultez les logs !'}"
            )
            return
        LOGGER.info(f"Leech terminé : {self._listener.name}")
    ...
    async def _upload_file(self, cap_mono, file, o_path, force_document=False):
        ...
        try:
            ...
            LOGGER.error(f"{err_type}{err}. Chemin: {self._up_path}")
            if isinstance(err, BadRequest) and key != "documents":
                LOGGER.error(f"Retentative en tant que document. Chemin: {self._up_path}")
    ...
    async def cancel_task(self):
        ...
        LOGGER.info(f"Annulation de l'envoi : {self._listener.name}")
        await self._listener.on_upload_error("Votre envoi a été arrêté !")