...
class YtSelection:
    ...
    async def get_quality(self, result):
        ...
        if "entries" in result:
            ...
            buttons.data_button("Formats audio", "ytq audio")
            buttons.data_button("Meilleures vidéos", "ytq bv*+ba/b")
            buttons.data_button("Meilleurs audios", "ytq ba/b")
            buttons.data_button("Annuler", "ytq cancel", "footer")
            self._main_buttons = buttons.build_menu(3)
            msg = f"Qualité des vidéos de la playlist :\nDélai : {get_readable_time(self._timeout - (time() - self._time))}"
        else:
            ...
            buttons.data_button("MP3", "ytq mp3")
            buttons.data_button("Formats audio", "ytq audio")
            buttons.data_button("Meilleure vidéo", "ytq bv*+ba/b")
            buttons.data_button("Meilleur audio", "ytq ba/b")
            buttons.data_button("Annuler", "ytq cancel", "footer")
    ...
    async def qual_subbuttons(self, b_name):
        ...
        buttons.data_button("Retour", "ytq back", "footer")
        buttons.data_button("Annuler", "ytq cancel", "footer")
        subbuttons = buttons.build_menu(2)
        msg = f"Débit binaire pour <b>{b_name}</b> :\nDélai : {get_readable_time(self._timeout - (time() - self._time))}"
    ...
    async def mp3_subbuttons(self):
        ...
        buttons.data_button("Retour", "ytq back")
        buttons.data_button("Annuler", "ytq cancel")
        subbuttons = buttons.build_menu(3)
        msg = f"Débit audio MP3{i} :\nDélai : {get_readable_time(self._timeout - (time() - self._time))}"
        await edit_message(self._reply_to, msg, subbuttons)
    ...
    async def audio_quality(self, format):
        ...
        buttons.data_button("Retour", "ytq aq back")
        buttons.data_button("Annuler", "ytq aq cancel")
        subbuttons = buttons.build_menu(5)
        msg = f"Qualité audio{i} :\n0 = meilleure, 10 = pire\nDélai : {get_readable_time(self._timeout - (time() - self._time))}"
        await edit_message(self._reply_to, msg, subbuttons)
...
class YtDlp(TaskListener):
    ...
    async def new_event(self):
        ...
        LOGGER.info(f"Téléchargement avec YT-DLP : {self.link}")
