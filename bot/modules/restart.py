...
async def restart_bot(_, message):
    ...
    buttons.data_button("Oui !", "botrestart confirm")
    buttons.data_button("Annuler", "botrestart cancel")
    button = buttons.build_menu(2)
    await send_message(message, "Êtes-vous sûr de vouloir redémarrer le bot ?", button)


async def send_incomplete_task_message(cid, msg_id, msg):
    try:
        if msg.startswith("Redémarré avec succès !"):
...
async def restart_notification():
    ...
    if Config.INCOMPLETE_TASK_NOTIFIER and Config.DATABASE_URL:
        if notifier_dict := await database.get_incomplete_tasks():
            for cid, data in notifier_dict.items():
                msg = "Redémarré avec succès !" if cid == chat_id else "Bot redémarré !"
    ...
    if await aiopath.isfile(".restartmsg"):
        try:
            await TgClient.bot.editTextMessage(
                chat_id=chat_id, message_id=msg_id, text="Redémarré avec succès !"
            )
        except:
            pass
        await remove(".restartmsg")
...
async def confirm_restart(_, query):
    ...
    if data[1] == "confirm":
        ...
        restart_message = await send_message(reply_to, "Redémarrage en cours...")
        await TgClient.stop()
