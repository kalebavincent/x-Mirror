...
async def get_buttons(key=None, edit_type=None):
    buttons = ButtonMaker()
    if key is None:
        buttons.data_button("Variables de configuration", "botset var")
        buttons.data_button("Fichiers privés", "botset private")
        buttons.data_button("Paramètres Qbit", "botset qbit")
        buttons.data_button("Paramètres Aria2c", "botset aria")
        buttons.data_button("Paramètres Sabnzbd", "botset nzb")
        buttons.data_button("Synchronisation JDownloader", "botset syncjd")
        buttons.data_button("Fermer", "botset close")
        msg = "Paramètres du bot :"
    elif edit_type is not None:
        if edit_type == "botvar":
            msg = ""
            buttons.data_button("Retour", "botset var")
            if key not in ["TELEGRAM_HASH", "TELEGRAM_API", "OWNER_ID", "BOT_TOKEN"]:
                buttons.data_button("Par défaut", f"botset resetvar {key}")
            buttons.data_button("Fermer", "botset close")
            if key in [
                "CMD_SUFFIX",
                "OWNER_ID",
                ...
                msg += "Redémarrage requis pour cette modification ! Vous ne verrez pas les changements dans les variables du bot, l'édition sera uniquement dans la base de données !\n\n"
            msg += f"Envoyez une valeur valide pour {key}. Valeur actuelle : '{Config.get(key)}'. Délai : 60 sec"
        elif edit_type == "ariavar":
            buttons.data_button("Retour", "botset aria")
            if key != "newkey":
                buttons.data_button("Chaîne vide", f"botset emptyaria {key}")
            buttons.data_button("Fermer", "botset close")
            msg = (
                "Envoyez une clé avec sa valeur. Exemple : https-proxy-user:valeur. Délai : 60 sec"
                if key == "newkey"
                else f"Envoyez une valeur valide pour {key}. Valeur actuelle : '{aria2_options[key]}'. Délai : 60 sec"
            )
        elif edit_type == "qbitvar":
            buttons.data_button("Retour", "botset qbit")
            buttons.data_button("Chaîne vide", f"botset emptyqbit {key}")
            buttons.data_button("Fermer", "botset close")
            msg = f"Envoyez une valeur valide pour {key}. Valeur actuelle : '{qbit_options[key]}'. Délai : 60 sec"
        elif edit_type == "nzbvar":
            buttons.data_button("Retour", "botset nzb")
            buttons.data_button("Par défaut", f"botset resetnzb {key}")
            buttons.data_button("Chaîne vide", f"botset emptynzb {key}")
            buttons.data_button("Fermer", "botset close")
            msg = f"Envoyez une valeur valide pour {key}. Valeur actuelle : '{nzb_options[key]}'.\nSi la valeur est une liste, séparez les éléments par des espaces ou des virgules\nExemple : .exe,info ou .exe .info\nDélai : 60 sec"
        elif edit_type.startswith("nzbsevar"):
            index = 0 if key == "newser" else int(edit_type.replace("nzbsevar", ""))
            if key == "newser":
                buttons.data_button("Retour", "botset nzbserver")
                msg = "Envoyez un serveur sous forme de dictionnaire {}, comme dans config.py sans []. Délai : 60 sec"
            else:
                buttons.data_button("Vider", f"botset emptyserkey {index} {key}")
                buttons.data_button("Retour", f"botset nzbser{index}")
                msg = f"Envoyez une valeur valide pour {key} dans le serveur {Config.USENET_SERVERS[index]['name']}. Valeur actuelle : {Config.USENET_SERVERS[index][key]}. Délai : 60 sec"
            buttons.data_button("Fermer", "botset close")
    elif key == "var":
        ...
        if state == "view":
            buttons.data_button("Modifier", "botset edit var")
        else:
            buttons.data_button("Voir", "botset view var")
        buttons.data_button("Retour", "botset back")
        buttons.data_button("Fermer", "botset close")
        for x in range(0, len(conf_dict), 10):
            buttons.data_button(
                f"{int(x / 10)}", f"botset start var {x}", position="footer"
            )
        msg = f"Variables de configuration | Page : {int(start / 10)} | État : {state}"
    elif key == "private":
        buttons.data_button("Retour", "botset back")
        buttons.data_button("Fermer", "botset close")
        msg = """Envoyez un fichier privé : config.py, token.pickle, rclone.conf, accounts.zip, list_drives.txt, cookies.txt, .netrc ou tout autre fichier privé !
Pour supprimer un fichier privé, envoyez uniquement le nom du fichier en message texte.
Note : Changer .netrc ne prendra pas effet pour aria2c avant redémarrage.
Délai : 60 sec"""
    elif key == "aria":
        ...
        if state == "view":
            buttons.data_button("Modifier", "botset edit aria")
        else:
            buttons.data_button("Voir", "botset view aria")
        buttons.data_button("Ajouter une clé", "botset ariavar newkey")
        buttons.data_button("Retour", "botset back")
        buttons.data_button("Fermer", "botset close")
        ...
        msg = f"Options Aria2c | Page : {int(start / 10)} | État : {state}"
    elif key == "qbit":
        for k in list(qbit_options.keys())[start : 10 + start]:
            buttons.data_button(k, f"botset qbitvar {k}")
        if state == "view":
            buttons.data_button("Modifier", "botset edit qbit")
        else:
            buttons.data_button("Voir", "botset view qbit")
        buttons.data_button("Synchroniser Qbittorrent", "botset syncqbit")
        buttons.data_button("Retour", "botset back")
        buttons.data_button("Fermer", "botset close")
        ...
        msg = f"Options Qbittorrent | Page : {int(start / 10)} | État : {state}"
    elif key == "nzb":
        for k in list(nzb_options.keys())[start : 10 + start]:
            buttons.data_button(k, f"botset nzbvar {k}")
        if state == "view":
            buttons.data_button("Modifier", "botset edit nzb")
        else:
            buttons.data_button("Voir", "botset view nzb")
        buttons.data_button("Serveurs", "botset nzbserver")
        buttons.data_button("Synchroniser Sabnzbd", "botset syncnzb")
        buttons.data_button("Retour", "botset back")
        buttons.data_button("Fermer", "botset close")
        ...
        msg = f"Options Sabnzbd | Page : {int(start / 10)} | État : {state}"
    elif key == "nzbserver":
        ...
        buttons.data_button("Ajouter", "botset nzbsevar newser")
        buttons.data_button("Retour", "botset nzb")
        buttons.data_button("Fermer", "botset close")
        ...
        msg = f"Serveurs Usenet | Page : {int(start / 10)} | État : {state}"
        ...
        if state == "view":
            buttons.data_button("Modifier", f"botset edit {key}")
        else:
            buttons.data_button("Voir", f"botset view {key}")
        buttons.data_button("Supprimer le serveur", f"botset remser {index}")
        buttons.data_button("Retour", "botset nzbserver")
        buttons.data_button("Fermer", "botset close")
        ...
        msg = f"Clés du serveur | Page : {int(start / 10)} | État : {state}"
...
async def edit_nzb_server(_, message, pre_message, key, index=0):
    ...
    if key == "newser":
        ...
        if res["config"]["servers"][0][key] == "":
            await send_message(message, "Valeur invalide")
            return
...
async def update_private_file(_, message, pre_message):
    ...
    if not message.remote_file_id and (file_name := message.text):
        ...
        if "@github.com" in Config.UPSTREAM_REPO:
            buttons = ButtonMaker()
            msg = "Pousser vers UPSTREAM_REPO ?"
            buttons.data_button("Oui !", f"botset push {file_name}")
            buttons.data_button("Non", "botset close")
...
async def edit_bot_settings(client, query):
    ...
    if data[1] == "close":
        ...
        if not Config.JD_EMAIL or not Config.JD_PASS:
            await query.answer(
                "Aucun email ou mot de passe fourni !",
                show_alert=True,
            )
            return
        await query.answer(
            "Synchronisation démarrée. JDownloader va redémarrer. Cela prend jusqu'à 10 sec !",
            show_alert=True,
        )
