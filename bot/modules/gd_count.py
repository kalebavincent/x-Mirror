...
async def count_node(_, message):
    ...
    if is_gdrive_link(link):
        msg = await send_message(message, f"Calcul en cours : <code>{link}</code>")
        ...
        msg = f"<b>Nom : </b><code>{name}</code>"
        msg += f"\n\n<b>Taille : </b>{get_readable_file_size(size)}"
        msg += f"\n\n<b>Type : </b>{mime_type}"
        if mime_type == "Folder":
            msg += f"\n<b>Sous-dossiers : </b>{folders}"
            msg += f"\n<b>Fichiers : </b>{files}"
        msg += f"\n\n<b>Destinataire : </b>{tag}"
    else:
        msg = (
            "Envoyez un lien Google Drive avec la commande "
            "ou en répondant au lien avec la commande"
        )
